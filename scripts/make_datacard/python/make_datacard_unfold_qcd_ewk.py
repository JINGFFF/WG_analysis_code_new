...
for i in range(1,binNum+1):
    ...
    bin_content = other.GetBinContent(i) + vbs_gensum + VBS_out.GetBinContent(i) + fakephoton.GetBinContent(i) + fakelepton.GetBinContent(i) - doublefake.GetBinContent(i)
    ...
    for k1 in range(binNum):
        f.write('\tmubarrel')    
    f.write('\tmubarrel\tmubarrel\tmubarrel\tmubarrel\tmubarrel\n')

    f.write('process')
    for k3 in range(1,binNum+1):
       f.write('\tgenbin'+str(k3))
    f.write('\tVBS_out\tother\tfakephoton\tfakelepton\tdoublefake\n')
    ...
    f.write('\t%0.4f\t%0.4f\t%0.4f\t%0.4f\t%0.4f\n'%(VBS_out_bincontent,  other_bincontent, fakephoton_bincontent, fakelepton_bincontent, doublefake_bincontent))
    ...
    if year == '2016' or year == '2017':
        ...
        for ke in range(binNum):
            f.write('\t-')
        f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__L1[i - 1], other_L1[i - 1]))

    f.write('photon_ID\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__photon_ID[i - 1], other_photon_ID[i - 1]))

    f.write('electron_ID\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__electron_ID[i - 1], other_electron_ID[i - 1]))

    f.write('electron_Reco\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__electron_Reco[i - 1], other_electron_Reco[i - 1]))

    f.write('electron_HLT\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__electron_HLT[i - 1], other_electron_HLT[i - 1]))

    f.write('muon_ID\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__muon_ID[i - 1],  other_muon_ID[i - 1]))

    f.write('muon_iso\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__muon_iso[i - 1], other_muon_iso[i - 1]))

    f.write('muon_HLT\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__muon_HLT[i - 1], other_muon_HLT[i - 1]))
    ...
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__btag[i - 1], other_btag[i - 1]))

    f.write('JEC_'+year+'\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__JEC[i - 1], other_JEC[i - 1]))

    f.write('JER_'+year+'\tlnN')
    for ke in range(binNum):
        f.write('\t-')
    f.write('\t%0.4f\t%0.4f\t-\t-\t-\n'%(WGJJ_hist_out__JER[i - 1], other_JER[i - 1]))


    f.write('qcd_ewk_pdf'+'\tlnN')
    f.write('\t-'*(i-1))
    f.write('\t'+str(1+qcd_ewk_pdf[i-1])+ '\t-'*(binNum-1) + '\t'+str(1+qcd_ewk_pdf[i-1])+'\t-\t-\t-\t-\n')

    f.write('qcd_ewk_scale'+'\tlnN')
    f.write('\t-'*(i-1))
    f.write('\t'+str(1+qcd_ewk_scale[i-1])+ '\t-'*(binNum-1) + '\t'+str(1+qcd_ewk_scale[i-1])+'\t-\t-\t-\t-\n')
