...
histname = 'ptlep'
f_in_WGJJ = TFile.Open(indir + "/"+year+"_"+channel+"_mc_" + region + "_" + bORe + "_qcd_ewk.root")
...
for i in range(1,VBS_in.GetNbinsX()+1):
    ...
    f.write('process')
    f.write('\tVBS_in\tVBS_out\tother\tfakephoton\tfakelepton\tdoublefake\n')

    f.write('process')
    f.write('\t0\t1\t2\t3\t4\t5\n')

    f.write('rate')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\n'%(VBS_in_bincontent, VBS_out_bincontent, other_bincontent, fakephoton_bincontent, fakelepton_bincontent, doublefake_bincontent))
    ...
    if year == '2016' or year == '2017':
        f.write('L1'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__L1[i - 1], WGJJ_hist_out__L1[i - 1], other_L1[i - 1]))

    f.write('photon_ID'+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__photon_ID[i - 1], WGJJ_hist_out__photon_ID[i - 1],  other_photon_ID[i - 1]))

    if channel == 'electron' or channel == 'all':
        f.write('electron_ID'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__electron_ID[i - 1], WGJJ_hist_out__electron_ID[i - 1], other_electron_ID[i - 1]))

        f.write('electron_Reco'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__electron_Reco[i - 1], WGJJ_hist_out__electron_Reco[i - 1], other_electron_Reco[i - 1]))

        f.write('electron_HLT'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__electron_HLT[i - 1], WGJJ_hist_out__electron_HLT[i - 1], other_electron_HLT[i - 1]))
    if channel == 'muon' or channel == 'all':
        f.write('muon_ID'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__muon_ID[i - 1], WGJJ_hist_out__muon_ID[i - 1],  other_muon_ID[i - 1]))

        f.write('muon_iso'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__muon_iso[i - 1], WGJJ_hist_out__muon_iso[i - 1], other_muon_iso[i - 1]))

        f.write('muon_HLT'+'\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__muon_HLT[i - 1], WGJJ_hist_out__muon_HLT[i - 1],  other_muon_HLT[i - 1]))

    f.write('btag'+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__btag[i - 1], WGJJ_hist_out__btag[i - 1], other_btag[i - 1]))

    f.write('JEC_'+year+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__JEC[i - 1], WGJJ_hist_out__JEC[i - 1], other_JEC[i - 1]))

    f.write('JER_'+year+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__JER[i - 1], WGJJ_hist_out__JER[i - 1],  other_JER[i - 1]))
...
#
    ...
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_hist_in__pileup[i - 1], WGJJ_hist_out__pileup[i - 1], other[i - 1]))
    ...
    f.write('qcd_ewk_pdf'+'\tlnN')
    f.write('\t'+str(1+WGJJ_pdf[i-1])+'\t'+str(1+WGJJ_pdf[i-1])+'\t-\t-\t-\t-\n')

    f.write('qcd_ewk_scale'+'\tlnN')
    ...
    f.write("Theory group = qcd_ewk_pdf qcd_ewk_scale " + '\n')
