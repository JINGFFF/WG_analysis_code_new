...
f_in_WGJJ_m_b   = TFile.Open(inputdir + "/" + year + "_WGJJ.root")

f_in_WGJets_m_b = TFile.Open(inputdir + "/" + year + "_WGJets.root")

#f_in_WGQCDEWK_m_b = TFile.Open(inputdir + "/" + year + "_" + channel + "_mc_" + region + "_" + eORb + "_WGQCDEWK.root")

#ll = [1, 2, 3, 4]

def get_bincontent(f):
    h = f.Get("hist_")
    h.Print()
    bin_content = []
    for i in range(h.GetNbinsX()):
        bin_content.append(h.GetBinContent(i+1))
    return bin_content
...
#if shape == '1': 
bin_content_WGJJ   = get_bincontent(f_in_WGJJ_m_b)
bin_content_WGJets = get_bincontent(f_in_WGJets_m_b)

outfile = open(outputdir + "/uncer_pdf.txt", 'w')
for i in range(len(bin_content_WGJJ)):
    error = ( (ll_wgjj_pdf[i]*bin_content_WGJJ[i])**2 + (ll_wgjj_scale[i]*bin_content_WGJJ[i])**2 + (ll_wgjets_pdf[i]*bin_content_WGJets[i])**2 + (ll_wgjets_muR1[i]*bin_content_WGJets[i])**2 + (ll_wgjets_muF1[i]*bin_content_WGJets[i])**2 + (ll_wgjets_muRmuF[i]*bin_content_WGJets[i])**2)**0.5/(bin_content_WGJJ[i]+bin_content_WGJets[i])
    outfile.write( str(error) +'\n')
'''
outPYfile = open(outputdir + "/uncer_pdf.py", 'w')
#if shape == '0':
