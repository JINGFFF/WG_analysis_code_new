...
void test::Loop(TDirectory * dir, TTree * tree)
{
   ...
   while (fReader.Next()) {
      ...
      if (jentry % 10000 == 0){ 
         ...
         string s1(ks,'>');
         string s2(50-ks,'-');
         cout<<"processing : ["<<"\033[32m"<<s1<<"\033[0m"<<s2<<"]  "<<"\033[33m"<<100.*jentry/maxEntries<<"%  "<<" pass gen: "<<nacc<<" "<<abnacc<<" "<<"p_event: "<<p_event<<" n_event: "<<n_event<<"\033[0m"<<endl;
...
if(cut_gen) {
nacc  = nacc +1;
abnacc = abnacc + fabs(*theWeight)/(*theWeight);//cout<<nacc<<endl;
}
