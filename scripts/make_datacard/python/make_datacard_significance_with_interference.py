...
f_in_QCDandInterference = TFile.Open(indir + "/"+year+"_"+channel+"_mc_" + region + "_" +bORe + "_QCDandInterference.root")
QCDandInterference = f_in_QCDandInterference.Get("hist_")
QCDandInterference.Scale(lumi)
...
for i in range(1,VBS.GetNbinsX()+1):
    ...
    bin_content = other.GetBinContent(i) + QCDandInterference.GetBinContent(i)   + VBS.GetBinContent(i) + fakephoton.GetBinContent(i) + fakelepton.GetBinContent(i) - doublefake.GetBinContent(i)
    # bincontent of each precess
    other_bincontent = other.GetBinContent(i) if other.GetBinContent(i)>0 else 0
    QCDandInterference_bincontent = QCDandInterference.GetBinContent(i) if QCDandInterference.GetBinContent(i)>0 else 0
    ...
    QCDandInterference_binerror = QCDandInterference.GetBinError(i)/QCDandInterference_bincontent if QCDandInterference_bincontent>0 else 0
    QCDandInterference_binerror = QCDandInterference_binerror if QCDandInterference_binerror<1 else 1
    QCDandInterference_binerror = QCDandInterference_binerror+1
    ...
    f.write('process')
    f.write('\tVBS\tQCDandInterference\tother\tfakephoton\tfakelepton\tdoublefake\n')

    f.write('process')
    f.write('\t0\t1\t2\t3\t4\t5\n')

    f.write('rate')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\n'%(VBS_bincontent, QCDandInterference_bincontent, other_bincontent, fakephoton_bincontent, fakelepton_bincontent, doublefake_bincontent))
    ...
    #for k in range(1,VBS.GetNbinsX()+1):
       ...
       #elif k != i :
        #   f.write('\t-')
    #f.write('\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(VBS_out_binerror, QCDandInterference_binerror, ST_s_binerror, ST_t_binerror, ST_tbar_binerror, ST_tW_binerror, ST_tbarW_binerror, TTG_binerror, WW_binerror, WZ_binerror, ZZ_binerror, ZG_binerror))
    f.write('WGJJ_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i)  + '\tlnN')
    f.write('\t'+str(VBS_binerror)+'\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\n')

    f.write('QCDandInterference_'+year+'_' + region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + '\tlnN')
    f.write('\t-\t'+str(QCDandInterference_binerror)+'\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\n')
    ...
    if QCDandInterference_bincontent != 0 :
        f.write('QCDandInterference_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ')
    ...
    f.write('\n')
     #+ 'QCDandInterference_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'other_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'fakephoton_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'fakelepton_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'doublefake_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + '\n')


    f.write('photon_ID\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_photon_ID[i - 1], QCDandInterference_photon_ID[i - 1], other_photon_ID[i - 1]))
    f.write("photon group = photon_ID" + '\n')

    if year == '2016' or year == '2017':
        f.write('L1\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_L1[i - 1], QCDandInterference_L1[i - 1], other_L1[i - 1]))
        f.write("L1pre group = L1" + '\n')

    if channel == 'electron' or channel == 'all':
        f.write('electron_ID\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_electron_ID[i - 1], QCDandInterference_electron_ID[i - 1], other_electron_ID[i - 1]))

        f.write('electron_Reco\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_electron_Reco[i - 1], QCDandInterference_electron_Reco[i - 1], other_electron_Reco[i - 1]))

        f.write('electron_HLT\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_electron_HLT[i - 1], QCDandInterference_electron_HLT[i - 1], other_electron_HLT[i - 1]))
        f.write("electron group = electron_ID electron_Reco electron_HLT" + '\n')
    
    if channel == 'muon' or channel == 'all':
        f.write('muon_ID\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_muon_ID[i - 1], QCDandInterference_muon_ID[i - 1], other_muon_ID[i - 1]))

        f.write('muon_iso\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_muon_iso[i - 1], QCDandInterference_muon_iso[i - 1], other_muon_iso[i - 1]))

        f.write('muon_HLT\tlnN')
        f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_muon_HLT[i - 1], QCDandInterference_muon_HLT[i - 1], other_muon_HLT[i - 1]))
        f.write("muon group = muon_ID muon_iso muon_HLT" + '\n')

    f.write('btag\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_btag[i - 1], QCDandInterference_btag[i - 1], other_btag[i - 1]))
    f.write("Btag group = btag" + '\n')

    f.write('JEC_'+year+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_JEC[i - 1], QCDandInterference_JEC[i - 1], other_JEC[i - 1]))

    f.write('JER_'+year+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_JER[i - 1], QCDandInterference_JER[i - 1], other_JER[i - 1]))
    f.write("JECR group = " + 'JEC_'+year + ' ' + 'JER_'+year + '\n')

    #f.write('pujet_'+year+'\tlnN')
    #f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_pujet[i - 1], QCDandInterference_pujet[i - 1], other_pujet[i - 1]))

    #f.write('pujet_mistag_'+year+'\tlnN')
    #f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_pujet_mistag[i - 1], QCDandInterference_pujet_mistag[i - 1], other_pujet_mistag[i - 1]))

    f.write('pileup_'+year+'\tlnN')
    f.write('\t%0.5f\t%0.5f\t%0.5f\t-\t-\t-\n'%(WGJJ_pileup[i - 1], QCDandInterference_pileup[i - 1], other_pileup[i - 1]))
    ...
    #f.write("Stat group = " + 'WGJJ_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'QCDandInterference_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + '_' + 'other_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'fakephoton_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'fakelepton_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + ' ' + 'doublefake_' +year+'_' +region + '_' +channel+'_'+bORe+'_stat_bin_'+ str(i) + '\n')
    ...
    #f.write("Lumi grpup = " + 'lumi_'+year + '\n')

    f.write("Theory group = WGJJ_pdf WGJJ_scale WGJets_pdf WGJets_muF1 WGJets_muR1 WGJets_muRmuF" + '\n')

    #f.write("Noprompt group = " + 'fakephoton_'+bORe+'_'+year+ ' ' + 'fakelepton_'+channel+'_'+year + '\n')
